...
cat("survey_child_bin coding results (0 = Wave 1, 1 = Wave 2):\n")
...
cat("\nChange in the parent oral exostosis OR:\n")
cat(sprintf("  M0 crude OR                         : %.3f\n", or_progression[1]))
cat(sprintf("  M1 + offspring age/sex              : %.3f\n", or_progression[2]))
cat(sprintf("  M2 + parent age (primary model)     : %.3f\n", or_progression[3]))
cat(sprintf("  M3 + survey wave           : %.3f\n", or_progression[4]))

# Wave effect in M3 (for model-selection assessment)
cat("\n--- Survey-wave effect in M3 (assessment for inclusion) ---\n")
...
cat("If p > 0.05 and AIC does not decrease, use M2 as the final model; otherwise use M3.\n")

# 7. Detailed results for the primary model (M2)
cat("\n--- Detailed results for the primary model (M2) ---\n")
...
print(result_m2)
cat(sprintf("\nKey result: Parent oral exostosis aOR = %.3f (95%% CI: %.3f–%.3f), p = %.4f\n",
...
result_stratified_df <- do.call(rbind, lapply(pair_types, function(pt) {
  ...
  if (nrow(sub) < 30) { cat(sprintf("[%s] Sample size is too small; skipping.\n", pt)); return(NULL) }
  tryCatch({
  ...
  }, error=function(e){ cat(sprintf("[%s] Model failed: %s\n", pt, e$message)); NULL })
}))
...
cat("\nReminder: Verify the following after execution:\n")
cat("  1. The actual survey_child values match WAVE2_VALUES.\n")
cat("  2. Use the wave p-value and AIC in M3 to decide whether the final model should be M2 or M3.\n")
